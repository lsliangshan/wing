    ...
    >
      <template v-if="userInfo.unionId">
        ...
        <div class="text-white text-[16px] mt-2 flex flex-row">
          <div class="font-500 text-[#30DCAC]">
            {{ userInfo.nick || userInfo.enName }}
          </div>
 ...
 */
function getDingtalkUserInfo(params: { code: string }) {
  return new Promise((resolve, reject) => {
    axios
      .get(
        `https://wf.liangqy.com/webhook/getDingtalkUserInfoByAuthCode?code=${params.code}`
      )
      .then((res) => {
        resolve(res.data);
      })
...
function initLoginQrcode() {
  window.DTFrameLogin(
    ...
    {
      redirect_uri: encodeURIComponent("http://10.2.6.210:5173/dingtalk/login"),
      client_id: "dingeoe1icdmqtkgnn33",
      scope: "openid",
      response_type: "code",
      prompt: "consent",
    },
      ...
      getDingtalkUserInfo({ code: authCode }).then((res: any) => {
        if (res.code == 200) {
          ...
          if (window.flutter_inappwebview) {
            window.flutter_inappwebview.callHandler("dingtalkLoginCallback", {
              code: 200,
              data: { ...userInfo.value },
            });
          }
